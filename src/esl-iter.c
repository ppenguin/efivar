...
extern int esl_list_iter_next_with_size_correction(esl_list_iter *iter,
                                                   efi_guid_t *type,
...
int NONNULL(1, 2) esl_iter_new(esl_iter **iter, uint8_t *buf, size_t len) {
  int rc;

  if (len < sizeof(efi_signature_list_t) + sizeof(efi_signature_data_t)) {
    efi_error(
        "buffer is too small for any EFI_SIGNATURE_LIST entries: %zd < %zd",
...
int NONNULL(1) esl_iter_end(esl_iter *iter) {
  if (!iter) {
    errno = EINVAL;
    return -1;
  }
...
esl_iter_status_t NONNULL(1, 2, 3, 4, 5)
  ...
  if (iter->i == iter->nmemb) {
    ...
    iter->i = 0;
    if (correct_size)
      rc = esl_list_iter_next_with_size_correction(iter->iter, type, &iter->esd,
                                                   &iter->len, true);
    else
    ...
    if (!efi_guid_cmp(type, &efi_guid_x509_cert)) {
      ...
      asn1size =
          get_asn1_seq_size(iter->esd->signature_data,
                            iter->len - sizeof(iter->esd->signature_owner));
      debug("iter->len:%zu sizeof(owner):%zd bufsz:%zd asn1sz:%d", iter->len,
            sizeof(iter->esd->signature_owner),
            iter->len - sizeof(iter->esd->signature_owner), asn1size);

      if (asn1size < 0) {
        debug("iterator data claims to be an X.509 Cert but is not valid ASN.1 "
              "DER");
      } else if ((uint32_t)asn1size !=
                 iter->len - sizeof(iter->esd->signature_owner)) {
        debug("X.509 Cert ASN.1 size does not match signature_list Size (%d vs "
              "%zu)",
              asn1size, iter->len - sizeof(iter->esd->signature_owner));
      }
    }
    ...
    if ((sls - sizeof(efi_signature_list_t) - slh) % ss != 0) {
      efi_error("signature list size is not a multiple of the signature entry "
                "size: %zd %% %zd = %zd",
    ...
    debug("signature data entry (0x%zx %c 0x%zx) (%zd %c %zd)",
          esd_get_esl_offset(iter) + ss,
          LEG(esd_get_esl_offset(iter) + ss, bufsz), bufsz,
          esd_get_esl_offset(iter) + ss,
          LEG(esd_get_esl_offset(iter) + ss, bufsz), bufsz);
    if (esd_get_esl_offset(iter) + ss > bufsz) {
      errno = EOVERFLOW;
      debug("EFI_SIGNATURE_LIST is malformed");
      debug("signature data entry is not within list bounds (%zd > %zd) (0x%zx "
            "> 0x%zx)",
            esd_get_esl_offset(iter) + ss,
            bufsz + esd_get_esl_offset(iter) - ss,
...
esl_iter_status_t NONNULL(1, 2, 3, 4, 5)
    esl_iter_next(esl_iter *iter, efi_guid_t *type, efi_guid_t *owner,
                  uint8_t **data, size_t *len) {
  return esl_iter_next_with_size_correction(iter, type, owner, data, len,
                                            false);
}

int NONNULL(1) esl_iter_get_line(esl_iter *iter) {
  if (!iter) {
    errno = EINVAL;
    return -1;
  }
...
intptr_t NONNULL(1) esd_get_esl_offset(esl_iter *iter) {
...
int NONNULL(1) esl_list_iter_end(esl_list_iter *iter) {
  if (!iter) {
    errno = EINVAL;
    return -1;
  }
...
int NONNULL(1, 2, 3, 4)
    esl_list_iter_next_with_size_correction(esl_list_iter *iter,
                                            efi_guid_t *type,
                                            efi_signature_data_t **data,
                                            size_t *len, bool correct_size) {
  if (!iter) {
  ...
  if ((uint32_t)iter->offset >= iter->len) {
    efi_error("iter->offset (%jd) >= iter->len (%zd)", (intmax_t)iter->offset,
              iter->len);
    errno = EINVAL;
    return -1;
  }
  ...
  if (!iter->esl) {
    ...
    debug("list has %lu bytes left, element is %" PRIu32 "(0x%" PRIx32
          ") bytes",
          iter->len - iter->offset, iter->esl->signature_list_size,
          iter->esl->signature_list_size);

    if (iter->len - iter->offset < iter->esl->signature_list_size) {
      debug("EFI_SIGNATURE_LIST is malformed: len:%zd(0x%zx) offset:%zd(0x%zx) "
            "len-off:%zd(0x%zx) esl_size:%" PRIu32 "(0x%" PRIx32 ")",
            iter->len, iter->len, iter->offset, iter->offset,
    ...
    if (!efi_guid_cmp(&type, &efi_guid_x509_cert)) {
      ...
      asn1size =
          get_asn1_seq_size(iter->buf + iter->offset + sizeof(efi_guid_t),
      ...
      if (asn1size < 0) {
        ...
        debug("X.509 Cert ASN.1 size does not match signature_list_size (%d vs "
              "%zu)",
              asn1size, iter->esl->signature_size - sizeof(efi_guid_t));
      }
    }
    ...
    if ((uint32_t)iter->offset >= iter->len)
      return 0;
    iter->esl = (efi_signature_list_t *)((uintptr_t)(iter->buf + iter->offset));
  }
  ...
  debug(
      "signature list size:%d iter->len:%zd iter->offset:%zd signature_size:%u",
      iter->esl->signature_list_size, iter->len, iter->offset,
  ...
  size_t header_sz =
      sizeof(efi_signature_list_t) + iter->esl->signature_header_size;
...
int NONNULL(1, 2) esl_list_list_start(esl_list_iter *iter, void **buf) {
...
int NONNULL(1, 2) esl_list_list_size(esl_list_iter *iter, size_t *bufsz) {
  if (!iter) {
    errno = EINVAL;
    return -1;
  }
...
int NONNULL(1, 2) esl_list_header_size(esl_list_iter *iter, size_t *slh) {
  if (!iter || !iter->esl) {
    errno = EINVAL;
    return -1;
  }
...
int NONNULL(1, 2) esl_list_sig_size(esl_list_iter *iter, size_t *ss) {
  if (!iter || !iter->esl) {
    errno = EINVAL;
    return -1;
  }
...
int NONNULL(1, 2) esl_list_get_type(esl_list_iter *iter, efi_guid_t *type) {
  if (!iter || !iter->esl) {
    errno = EINVAL;
    return -1;
  }
